...
public interface PzWordRepository extends JpaRepository<PzWord, Integer> {
    ...
    /**
     * 힌트가 없는 활성화된 단어 조회 (페이징, 라라벨과 동일)
     */
    @Query("SELECT w FROM PzWord w WHERE w.isActive = true AND NOT EXISTS (SELECT 1 FROM PzHint h WHERE h.word.id = w.id) ORDER BY w.createdAt ASC")
    org.springframework.data.domain.Page<PzWord> findActiveWordsWithoutHints(org.springframework.data.domain.Pageable pageable);
